...
function canGo(path, to) {
    return to !== 'start' && (isUpper(to) || !path.find(p => p === to) || !hasGoneTwice(path))
}

function hasGoneTwice(path) {
    return path.some(p => !isUpper(p) && path.filter(pp => p === pp).length === 2)
}
...
console.log(finishedPaths.map(p => p.reverse().join(', ')).join('\n'))
